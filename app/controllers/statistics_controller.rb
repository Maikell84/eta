class StatisticsController < ApplicationController
  ...
  def index
    @grouped_consumptions = Consumption.left_joins(:weather)
                                       .includes(:weather)
                                       .where('consumptions.created_at >= ?', 31.days.ago.beginning_of_day)
                                       .where('consumptions.created_at > ?', Date.parse('2021-03-19'))
